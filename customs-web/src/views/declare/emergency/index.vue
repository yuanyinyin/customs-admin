  ...
  <div class="LogContainer scroll-y">
    ...
    <el-card class="m-1">
      ...
      >
        ...
        <el-table-column prop="situation" label="事件状况" align="center" >
           <template #default="scope">
...
import {getEmergencyList,addEmergency,updateEmergency,deleteEmergency} from "@/api/emergency";
...
const getList = () =>{
    getEmergencyList(listQuery._rawValue)
    .then((response) => {
...
function userHandleSubmitAdd() {
  addEmergency(formData.value).then((res) => {
...
function userHandleSubmitEdit() {
  updateEmergency(formData.value.id, formData.value).then((res) => {
    userDialog.value.show = false
